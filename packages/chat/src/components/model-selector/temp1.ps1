...
export const KNOWN_PROVIDERS = Object.keys(PROVIDER_ICON_MAP)
...
export function getProviderIcon(model: ModelOption | string | null | undefined): Component | null {
  if (!model) {
    return null
  }
  ...
  if (typeof model === ''object'') {
    ...
    return providerKey ? PROVIDER_ICON_MAP[providerKey] : null
  }
...
'@
Set-Content -LiteralPath 'packages/chat/src/components/model-selector/providerIconMap.ts' -Value $content -Encoding utf8
'@